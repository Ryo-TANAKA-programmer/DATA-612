...
VaxData <- read.csv("time_series_covid19_vaccine_global.csv")
 ...
 # plot stacked bar chart of "freedom_status" by "region"
  ...
  ## Regression Analysis ##
  est1 <- lm(VaxxedPercentage ~ State_Illegitimacy + Region, data = ProjectData)
  est2 <- lm(VaxxedPercentage ~ Freedom_Status + Region, data = ProjectData)
  est3 <- lm(VaxxedPercentage ~ Civil_Liberties + Region, data = ProjectData)
  est4 <- lm(VaxxedPercentage ~ FOE + Region, data = ProjectData)
